...
function createWebRTCInput() {
  ...
  if (shareMode === "device") {
    input
      .getUserMedia(getDeviceConstraints())
...
function readyStreaming() {
  ...
  if (shareMode === "device" || shareMode === "display") {
    ...
    if (shareMode === "device") {
      beforeStreamingTitle.text(
        "Click start button to share your WebCam / Mic"
      );
  ...
  if (shareMode === "rtmp") {
    ...
    srtInputUrlInput.val(
      OME_SRT_INPUT_URL + encodeURIComponent(selectedInputStreamName)
    );

    const whipInputUrl =
      OME_WEBRTC_INPUT_HOST.replace("ws", "http") +
      "/" +
      APP_NAME +
      "/" +
      selectedInputStreamName +
      "?direction=whip&transport=tcp";
...
function startStreaming() {
  if (selectedInputStreamName && liveKitInputMap[selectedInputStreamName]) {
    ...
    liveKitInputMap[selectedInputStreamName].startStreaming(
      OME_WEBRTC_INPUT_HOST +
        "/" +
        APP_NAME +
        "/" +
        selectedInputStreamName +
...
function createLocalPlayer(streamName) {
  ...
  document.getElementById("local-player-" + streamName).srcObject =
    liveKitInputMap[streamName].inputStream;
}
...
function createPlayer(streamName) {
  ...
  const player = OvenPlayer.create(
    document.getElementById("player-" + streamName),
    playerOption
  );
  ...
  if (resp.statusCode === 200) {
    ...
    streams.forEach((streamName, index) => {
      ...
      if (!currentStreams.includes(streamName)) {
        ...
        if (shareMode === "rtmp" && streamName === selectedInputStreamName) {
